...
class PSRLS(tk.Frame):
    def __init__(self, parent,*args, **kwargs):
        ...
        _move =  ["Rock","Paper","Scissors", "Spock", "Lizard"] # make a temperary list of move
        ...
        _pictures = self.GetPictures() #calls a class function of GetPictures which will either grab the pictures from the local directory or from the internet (my github repository)

        for i in range(0,5):
            Move(self, _move[i], i, _binds[i], _pictures[i]).pack()
    ...
    def GetPictures(self):
        __ImgNames =  ["Rock","Paper","Scissors", "Spock", "Lizard","i_icon","setting_icon","Waiting"] #temperary variable for the names of pictures that need to be added
        __ImgObjects = []
        print("Loading Pictures...")
        for Img in _ImgNames:
            try:
                _ImgObjects.append(ImageTk.PhotoImage(Image.open(Img+".png")))
            except:
                try:
                    import urllib.request as urllib
                    import io
                    _ImgObjects.append(ImageTk.PhotoImage(Image.open(io.BytesIO(urllib.urlopen("https://raw.githubusercontent.com/James712346/Rock-Paper-Scissors-and-the-Bang/master/"+Img+".png").read()))))
                except:
                    _ImgObjects.append("Failed")
                    print("Failed to load",Img+".png")
